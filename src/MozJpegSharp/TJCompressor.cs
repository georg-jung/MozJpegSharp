...
namespace MozJpegSharp
{
    ...
    public class TJCompressor : IDisposable
    {
        ...
        /// </list>
        /// </exception>
        public unsafe Span<byte> Compress(ReadOnlySpan<byte> srcBuf, Span<byte> destBuf, int stride, int width, int height, PixelFormat pixelFormat, TJSubsamplingOption subSamp, int quality, TJFlags flags)
        {
            var tjPixelFormat = TJUtils.ConvertPixelFormat(pixelFormat);
